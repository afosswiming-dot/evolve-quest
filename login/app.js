...
async function loadJourneyStage() {
  const { data: stage, error } = await client.rpc('get_my_journey_stage');

  if (error) {
    const message = String(error.message || '').toUpperCase();
    if (message.includes('PROFILE_NOT_FOUND_OR_INACTIVE')) {
      throw new Error('PROFILE_NOT_FOUND_OR_INACTIVE');
    }
  ...
  const destination = CONFIG.routes[stage] || CONFIG.routes.welcome;
...
function authErrorMessage(error) {
  ...
  if (text.includes('account_inactive') || text.includes('profile_not_found_or_inactive')) return 'Seu perfil não pôde ser validado. Atualize a página ou fale com seu Healer.';
  if (text.includes('not_authenticated')) return 'Sua sessão expirou. Entre novamente.';
...
loginForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    const { destination } = await loadJourneyStage();

  ...
  async function loadDashboard(userId) {
    ...
    const [profile, journey, assignments, records, checkpoint, subscription, evaluation] = await Promise.all([
      client.from('profiles')
        .select('id, full_name, preferred_name, account_status, access_status, profile_status, journey_stage, current_level, class_id, current_chapter_id, healer_id')
        .eq('id', userId).maybeSingle(),
      client.from('adventurer_journeys')
        .select('id, status, current_chapter_id, current_level, xp_enabled, total_xp, chapter_progress, current_streak, started_at')
        .eq('adventurer_id', userId)
        .in('status', ['locked','active','paused'])
        .order('created_at', { ascending: false }).limit(1).maybeSingle(),
      client.from('mission_assignments')
        .select('id, journey_id, chapter_id, mission_id, mission_type, status, available_at, completed_at, missions(name, objective, estimated_duration_minutes, mission_type, environment, subtitle)')
        .eq('adventurer_id', userId)
    ...
    // Resolve Classe and Capítulo explicitly instead of depending on embedded joins.
    if (state.profile?.class_id) {
      const classResponse = await client
        .from('adventurer_classes')
        .select('id, name')
        .eq('id', state.profile.class_id)
        .maybeSingle();
      if (classResponse.error) throw classResponse.error;
      state.profile.adventurer_classes = classResponse.data;
    }

    const chapterId = state.journey?.current_chapter_id || state.profile?.current_chapter_id;
    if (chapterId) {
      const chapterResponse = await client
        .from('chapters')
        .select('id, title, chapter_number')
        .eq('id', chapterId)
        .maybeSingle();
      if (chapterResponse.error) throw chapterResponse.error;
      state.profile.chapters = chapterResponse.data;
      if (state.journey) state.journey.chapters = chapterResponse.data;
    }

    // The next action is always an assignment officially prescribed by the Healer.
    state.assignment =

(() => {
  ...
  async function loadAuthenticatedContext() {
    ...
    if (!session?.user) {
      ...
      window.setTimeout(() => window.location.assign(CONFIG.loginRoute), 1200);
      return false;
    }
  ...
  async function startEvaluation() {
    ...
    try {
      await client.from('profiles').update({ journey_stage: 'assessment' }).eq('id', currentUser.id);

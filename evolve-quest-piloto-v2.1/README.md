# EVOLVE Quest — Piloto v2

Fluxo oficial do Aventureiro:

1. Login
2. Cadastro
3. Escolha da Jornada
4. Pagamento
5. Boas-vindas
6. Avaliação Inicial
7. Tela de espera
8. Painel do Aventureiro
9. Missões
10. Registro da Missão
11. Checkpoint
12. Progressão

O Painel do Healer permanece separado, com autenticação e rota administrativas próprias.

## Rotas atualmente integradas

- `/login/`
- `/cadastro/`
- `/escolha-jornada/`
- `/boas-vindas/`
- `/avaliacao-inicial/`

## Rotas preparadas para módulos futuros

- `/tela-espera/`
- `/painel-aventureiro/`
- `/missoes/`
- `/registro-missao/`
- `/checkpoint/`
- `/progressao/`
- `/admin/healer/`

## Correção v2.1

- Confirmação de e-mail retorna para `/login/`.
- O Login detecta a sessão criada pelo link de confirmação.
- Aventureiros autenticados são encaminhados automaticamente para a etapa correta.
- Autorize `https://evolve-quest.vercel.app/login/` nas Redirect URLs do Supabase.

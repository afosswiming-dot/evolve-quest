...
async function signUpAventureiro(payload) {
  const response = await fetch(`${APP_CONFIG.supabase.url}/auth/v1/signup`, {
    method: 'POST',
    headers: {
...
function navigateToJourneySelection() {
  const target = APP_CONFIG.routes.journeySelection;
...
async function handleSubmit(event) {
  ...
  try {
    ...
    if (authenticated) {
      showStatus('success', 'Acesso criado. Agora escolha como deseja iniciar sua jornada.');
      window.setTimeout(navigateToJourneySelection, APP_CONFIG.autoRedirectDelay);
    } else {

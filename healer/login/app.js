 ...
 * Configure as variáveis abaixo antes da publicação.
 */
...
async function signOutSafely() {
  ...
  try { await supabaseClient.auth.signOut(); } catch (error) { console.error('Falha ao encerrar sessão:', error); }
}
...
async function handleLogin(event) {
  ...
  if (!isConfigured) {
    setStatus(elements.status, 'A integração com o Supabase ainda precisa ser configurada no arquivo app.js.', 'info');
    return;
  }
  ...
  try {
    ...
    if (!elements.remember.checked) {
      sessionStorage.setItem('evolve-healer-session-only', 'true');
    } else {
      sessionStorage.removeItem('evolve-healer-session-only');
    }
...
async function handleRecovery(event) {
  ...
  if (!isConfigured) {
    setStatus(elements.recoveryStatus, 'A integração com o Supabase ainda precisa ser configurada no arquivo app.js.', 'info');
    return;
  }
  ...
  try {
    ...
    setStatus(elements.recoveryStatus, 'Não foi possível enviar as instruções agora. Tente novamente.', 'error');
  } finally {
...
function initializeInterface() {
  ...
  window.addEventListener('beforeunload', () => {
    if (sessionStorage.getItem('evolve-healer-session-only') === 'true') {
      // A sessão é encerrada na próxima inicialização caso o navegador tenha sido fechado.
      sessionStorage.setItem('evolve-healer-close-pending', 'true');
    }
  });
}
